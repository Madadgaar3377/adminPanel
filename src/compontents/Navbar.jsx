            ...
            <div className="max-w-7xl mx-auto px-3 xs:px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between h-14 sm:h-16 items-center">
                    ...
                    <Link to="/" className="flex items-center gap-2 min-w-0">
                        ...
                        <span className="text-lg sm:text-xl font-bold text-gray-900 hidden xs:block truncate">Madadgaar</span>
                    </Link>
                    ...
                    <div className="flex items-center gap-1 sm:gap-2">
                        {/* Chat */}
                        <Link to="/chat" className="p-1.5 sm:p-2 text-gray-500 hover:text-red-600 hover:bg-gray-50 rounded-lg transition-colors relative">
                        ...
                        {/* Notifications */}
                        <Link to="/notifications" className="p-1.5 sm:p-2 text-gray-500 hover:text-red-600 hover:bg-gray-50 rounded-lg transition-colors relative">
                            <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                            </svg>
                            <span className="absolute top-1.5 right-1.5 sm:top-2 sm:right-2 block h-1.5 w-1.5 sm:h-2 sm:w-2 rounded-full bg-red-500 ring-2 ring-white"></span>
                        </Link>
                        ...
                        <div className="relative">
                            <button
                                ...
                                className="flex items-center gap-1.5 sm:gap-2 p-1 rounded-lg hover:bg-gray-50 transition-colors"
                            >
                                <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-gray-900 flex items-center justify-center text-white font-semibold text-xs sm:text-sm flex-shrink-0">
                                    {adminData?.name ? adminData.name.substring(0, 2).toUpperCase() : 'AD'}
                                </div>
                                <span className="hidden lg:block text-sm font-medium text-gray-700 truncate max-w-[100px]">{adminData?.name || 'Admin'}</span>
                                <svg className={`hidden lg:block w-4 h-4 text-gray-500 transition-transform flex-shrink-0 ${isProfileOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                                </svg>
                            </button>
                            ...
                            {isProfileOpen && (
                                <>
                                    ...
                                    <div className="absolute right-0 mt-2 w-56 rounded-lg bg-white shadow-lg border border-gray-200 z-20">
                                        ...
                                        <div className="py-1">
                                            <Link to="/profile" onClick={() => setIsProfileOpen(false)} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                                                Profile
                                            </Link>
                                            <Link to="/update-password" onClick={() => setIsProfileOpen(false)} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                                                Change Password
                                            </Link>
                                        </div>
            ...
            {isMobileMenuOpen && (
                <div className="lg:hidden border-t border-gray-200 bg-white max-h-[calc(100vh-4rem)] overflow-y-auto">
                    <div className="px-3 xs:px-4 pt-2 pb-4 space-y-1">
                        ...
                        <Link to="/users" onClick={() => setIsMobileMenuOpen(false)} className="block px-3 py-2.5 rounded-lg text-sm font-medium text-gray-700 hover:text-red-600 hover:bg-gray-50">
                            Users
                        </Link>
                        {navigationCategories.map((cat, idx) => (
                            <div key={idx}>
                                <button
                                    ...
                                    className="w-full flex justify-between items-center px-3 py-2.5 rounded-lg text-sm font-medium text-gray-700 hover:text-red-600 hover:bg-gray-50 transition-colors"
                                >
                                    <span>{cat.title}</span>
                                    <svg className={`w-4 h-4 transition-transform flex-shrink-0 ${activeMobileMenu === idx ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                                    </svg>
                                </button>
                                {activeMobileMenu === idx && (
                                    <div className="pl-4 space-y-1 mt-1">
                                        {cat.items.map((item, i) => (
                                            <Link key={i} to={item.href} onClick={() => setIsMobileMenuOpen(false)} className="block px-3 py-2 rounded-lg text-sm text-gray-600 hover:text-red-600 hover:bg-red-50 transition-colors">
                                                {item.label}
                                            </Link>
                                        ))}
                                    </div>
