...
/** Recalculate EMI, markup, totals for one payment plan row */
...
export const submitPartnerCatalogPricing = async ({
  ...
  const profile = getEditorProfile(form);
...
/** Admin editor: partner meta from form (userId / company on behalf of partner) */
export const getEditorProfile = (form) => ({
  userId: form?.userId || "",
  companyName: form?.companyName || form?.companyNameOther || "Admin",
...
export const submitInstallmentPlanUpdate = async ({
  ...
  const profile = getEditorProfile(form);
  const uid = editorUserId || profile.userId || form.userId;
